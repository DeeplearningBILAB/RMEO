...
/***********************************************************
 *	Read the Raman probability.
 ****/
void ReadRamProb(FILE *File_Ptr, InputStruct *In_Ptr)
{
  char buf[STRLEN];

  /** read in Raman probability. **/
  strcpy(buf, FindDataLine(File_Ptr));
  if(buf[0]=='\0') 
	nrerror("Reading Raman probability.\n");
  sscanf(buf, "%lf", &In_Ptr->ramProb);
  if(In_Ptr->ramProb<=-0E-12) 
	nrerror("Nonpositive Raman probability..\n");
}
...

{
	...
	ReadRamProb(File_Ptr, In_Ptr);	/** Included by Vijitha et al. **/
	...
	OutStruct * Out_Ptr)
{
	...
	Out_Ptr->Tt    = 0.0;
	 ...
	 Out_Ptr->numRamPho=0; //input of raman photons

	/* Allocate the arrays and the matrices. */
	Out_Ptr->Rd_ra = AllocMatrix(0,nr-1,0,na-1);

	//editted: 
	Out_Ptr->rRd_ra1 = AllocMatrix(0,nr-1,0,na-1);
	Out_Ptr->rRd_ra2 = AllocMatrix(0,nr-1,0,na-1);
	...
	//editted: 
	Out_Ptr->Rdr_z  = AllocVector(0,nz-1);
	Out_Ptr->raman_xyz  = AllocMatrixl(0,nz-1,0,(nz*nz-1));
	Out_Ptr->Tt_ra = AllocMatrix(0,nr-1,0,na-1);

	//editted: 
	Out_Ptr->rTt_ra1 = AllocMatrix(0,nr-1,0,na-1);
	Out_Ptr->rTt_ra2 = AllocMatrix(0,nr-1,0,na-1);
...
void FreeData(InputStruct In_Parm, OutStruct * Out_Ptr)
{
	...
	//editted:
	FreeMatrix(Out_Ptr->rRd_ra1, 0,nr-1,0,na-1);
	FreeMatrix(Out_Ptr->rRd_ra2, 0,nr-1,0,na-1);
	...
	//editted: 
	FreeVector(Out_Ptr->Rdr_z, 0, nz-1);  //vector for Raman z coord 
	FreeMatrixl(Out_Ptr->raman_xyz, 0, nz-1, 0,(nz*nz-1));
	FreeMatrix(Out_Ptr->Tt_ra, 0, nr-1, 0, na-1);

	//editted:
	FreeMatrix(Out_Ptr->rTt_ra1, 0,nr-1,0,na-1);
	FreeMatrix(Out_Ptr->rTt_ra2, 0,nr-1,0,na-1);
...
void ScaleRdTt(InputStruct In_Parm, OutStruct *	Out_Ptr)
{
	short nr = In_Parm.nr;
	short na = In_Parm.na;
	short nz = In_Parm.nz; //editted for Z coord 
	double dr = In_Parm.dr;
	double da = In_Parm.da;
	double dz = In_Parm.dz;//editted for Z coord 
	short ir,ia;
	short iz; //editted for z coord 
	double scale1, scale2;
	double scale3;  //editted for z coord 
	...
	for(ir=0; ir<nr; ir++)  
		for(ia=0; ia<na; ia++) {
			...
			Out_Ptr->rRd_ra1[ir][ia] *= scale2;
			Out_Ptr->rRd_ra2[ir][ia] *= scale2;
			Out_Ptr->rTt_ra1[ir][ia] *= scale2;
			Out_Ptr->rTt_ra2[ir][ia] *= scale2;
		}
		...
		scale3 = 2.0*PI*dr*dr*dz*In_Parm.num_photons;	
		scale3 = 1.0/(dz*In_Parm.num_photons);
		for(iz=0; iz<nz; iz++) { 
		Out_Ptr->Rdr_z[iz] *= scale3;
	} 


}

void WriteRaman_xyz(FILE * file, 
			    short Nx, 
				short Ny, 
				short Nz,
			   OutStruct Out_Parm)
{
  ...
  fprintf(file, "%ld %ld %ld\n", Nx,Ny,Nz);
    ...
    for(iy=0;iy<NyNz;iy++)  
		for(ix=0;ix<Nx;ix++){
      fprintf(file, "%12.4E ", Out_Parm.raman_xyz[ix][iy]);
     // if( (ix*NyNz + iy + 1)%10 == 0) 
	  fprintf(file, "\n");
    }
...
void WriteVersion(FILE *file, char *Version)
{
	...
	fprintf(file, "# A_rz, Rd_ra,rRd_ra1, rRd_ra2, Tt_ra,  rTt_ra1, rTt_ra2 \n####\n\n");
}
...
void WriteRAT(FILE * file, OutStruct Out_Parm)
{
	...
	fprintf(file, 
	   ...
	   fprintf(file, 
	   "%ld \t#Total Raman photons [-]\n", Out_Parm.numRamPho); //write out data for Raman photons 
...
/***********************************************************
 **	5 numbers each line. rRd_ra1
 ****/
void WriteRRd_ra1(FILE * file, 
				short Nr,
				short Na,
				OutStruct Out_Parm)
{
  ...
  fprintf(file, 
	  "%s\n%s\n%s\n%s\n%s\n%s\n",	/* flag. */
	  "# rRd1[r][angle]. [1/(cm2sr)].",
	  "# rRd1[0][0], [0][1],..[0][na-1]",
	  "# rRd1[1][0], [1][1],..[1][na-1]",
	  "# ...",
	  "# rRd1[nr-1][0], [nr-1][1],..[nr-1][na-1]",
	  "rRd_ra1");
  
  for(ir=0;ir<Nr;ir++)
    for(ia=0;ia<Na;ia++)  {
      fprintf(file, "%12.4E ", Out_Parm.rRd_ra1[ir][ia]);
      if( (ir*Na + ia + 1)%5 == 0) fprintf(file, "\n");
    }
...
/***********************************************************
 **	5 numbers each line. rRd_ra2
 ****/
void WriteRRd_ra2(FILE * file, 
				short Nr,
				short Na,
				OutStruct Out_Parm)
{
  ...
  fprintf(file, 
	  "%s\n%s\n%s\n%s\n%s\n%s\n",	/* flag. */
	  "# rRd2[r][angle]. [1/(cm2sr)].",
	  "# rRd2[0][0], [0][1],..[0][na-1]",
	  "# rRd2[1][0], [1][1],..[1][na-1]",
	  "# ...",
	  "# rRd2[nr-1][0], [nr-1][1],..[nr-1][na-1]",
	  "rRd_ra2");
  
  for(ir=0;ir<Nr;ir++)
    for(ia=0;ia<Na;ia++)  {
      fprintf(file, "%12.4E ", Out_Parm.rRd_ra2[ir][ia]);
      if( (ir*Na + ia + 1)%5 == 0) fprintf(file, "\n");
    }
...
/***********************************************************
 **	5 numbers each line. rTt_ra1.
 ****/
void WriteRTt_ra1(FILE * file, 
				short Nr,
				short Na,
				OutStruct Out_Parm)
{
  ...
  fprintf(file, 
	  "%s\n%s\n%s\n%s\n%s\n%s\n",	/* flag. */
	  "# rTt1[r][angle]. [1/(cm2sr)].",
	  "# rTt1[0][0], [0][1],..[0][na-1]",
	  "# rTt1[1][0], [1][1],..[1][na-1]",
	  "# ...",
	  "# rTt1[nr-1][0], [nr-1][1],..[nr-1][na-1]",
	  "rTt_ra1");
  
  for(ir=0;ir<Nr;ir++)
    for(ia=0;ia<Na;ia++)  {
      fprintf(file, "%12.4E ", Out_Parm.rTt_ra1[ir][ia]);
      if( (ir*Na + ia + 1)%5 == 0) fprintf(file, "\n");
    }
...
/***********************************************************
 **	5 numbers each line. rTt_ra2.
 ****/
void WriteRTt_ra2(FILE * file, 
				short Nr,
				short Na,
				OutStruct Out_Parm)
{
  ...
  fprintf(file, 
	  "%s\n%s\n%s\n%s\n%s\n%s\n",	/* flag. */
	  "# rTt2[r][angle]. [1/(cm2sr)].",
	  "# rTt2[0][0], [0][1],..[0][na-1]",
	  "# rTt2[1][0], [1][1],..[1][na-1]",
	  "# ...",
	  "# rTt2[nr-1][0], [nr-1][1],..[nr-1][na-1]",
	  "rTt_ra2");
  
  for(ir=0;ir<Nr;ir++)
    for(ia=0;ia<Na;ia++)  {
      fprintf(file, "%12.4E ", Out_Parm.rTt_ra2[ir][ia]);
      if( (ir*Na + ia + 1)%5 == 0) fprintf(file, "\n");
    }
	...
	char * TimeReport)
{
    ...
    WriteRdr_z(file, In_Parm.nz, Out_Parm);  //editted for Z coord results 
	...
	//editted:
	//WriteRRd_ra1(file, In_Parm.nr, In_Parm.na, Out_Parm);
	//WriteRRd_ra2(file, In_Parm.nr, In_Parm.na, Out_Parm);

	//WriteTt_ra(file, In_Parm.nr, In_Parm.na, Out_Parm);

	//editted:
	//WriteRTt_ra1(file, In_Parm.nr, In_Parm.na, Out_Parm);
	//WriteRTt_ra2(file, In_Parm.nr, In_Parm.na, Out_Parm);
	WriteRaman_xyz(file, In_Parm.nz, In_Parm.nz, In_Parm.nz, Out_Parm);
